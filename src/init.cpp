...
gaspi_return_t lazygaspi_init(lazygaspi_id_t table_amount, lazygaspi_id_t table_size, gaspi_size_t row_size,
                              SizeDeterminer det_amount, SizeDeterminer det_tablesize,
                              SizeDeterminer det_rowsize){
    ...
    if(table_amount == 0) { 
        ...
        if(!(table_amount = det_amount(info->id, info->n))) return GASPI_ERR_INV_NUM;
    }
    if(table_size == 0) { 
        if(!det_tablesize) return GASPI_ERR_INV_NUM;
        if(!(table_size = det_tablesize(info->id, info->n))) return GASPI_ERR_INV_NUM;
    }
    if(row_size == 0) { 
        if(!det_rowsize) return GASPI_ERR_INV_NUM;
        if(!(row_size = det_rowsize(info->id, info->n))) return GASPI_ERR_INV_NUM;
    }
